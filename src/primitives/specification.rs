...
macro_rules! spec_to_generic {
    ($spec_id:expr, $e:expr) => {{
        match $spec_id {
            FRONTIER | SpecId::FRONTIER_THAWING => {
                use $crate::primitives::FrontierSpec as SPEC;
                $e
            }
            HOMESTEAD | SpecId::DAO_FORK => {
                use $crate::primitives::HomesteadSpec as SPEC;
                $e
            }
            TANGERINE => {
                use $crate::primitives::TangerineSpec as SPEC;
                $e
            }
            SPURIOUS_DRAGON => {
                use $crate::primitives::SpuriousDragonSpec as SPEC;
                $e
            }
            BYZANTIUM => {
                use $crate::primitives::ByzantiumSpec as SPEC;
                $e
            }
            PETERSBURG | CONSTANTINOPLE => {
                use $crate::primitives::PetersburgSpec as SPEC;
                $e
            }
            ISTANBUL | MUIR_GLACIER => {
                use $crate::primitives::IstanbulSpec as SPEC;
                $e
            }
            BERLIN => {
                use $crate::primitives::BerlinSpec as SPEC;
                $e
            }
            LONDON
            |ARROW_GLACIER
            |GRAY_GLACIER => {
                use $crate::primitives::LondonSpec as SPEC;
                $e
            }
            MERGE => {
                use $crate::primitives::MergeSpec as SPEC;
                $e
            }
            SHANGHAI => {
